...
class MiscCog(commands.Cog):
    ...
    async def prefix(self, ctx: Context, new_prefix: str) -> None:
        if new_prefix == 'reset': new_prefix = 'gn '
        await ctx.send(desc=f"Are you sure you wanna make the new guild prefix '{new_prefix}'")
        response: Message = await self.client.wait_for('message', check=lambda m: m.author == ctx.author)

        if response.content.startswith('y'):
            del self.client.prefix_cache[ctx.guild.id]
            await self.client.db.execute("""UPDATE guilds 
                                            SET prefix=$1 
                                            WHERE id=$2""", new_prefix, ctx.guild.id)
