...
class GuildCog(commands.Cog):
    ...
    async def leave(self, ctx: Context) -> None:
        ...
        confirmation: Message = await self.client.wait_for("message", check=lambda msg: msg.author == ctx.author)
        if confirmation.content.startswith("y"):
            await ctx.send(desc="Alright, leaving. \U0001f44c", reaction="\U00002705")
            await ctx.guild.leave()
        else:
            await ctx.send(desc="Didn't get an answer starting with 'y'. Guess I'll stay. \U0001f44c")
